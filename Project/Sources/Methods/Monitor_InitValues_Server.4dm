...
#DECLARE($what : Text)->$result : Object
...
Case of 
	...
	: ($1="application")
		$0:=Application info:C1599
		
	: ($1="license")
		$0:=License info:C1489
		
	: ($1="version")
		var $build : Integer
		var $version:=Application version:C493($build)
		
		$result:=New object:C1471("version"; $version; "build"; $build)
		
	: ($1="user")
		$0:=Process activity:C1495(Sessions only:K5:36)
		If ($0=Null:C1517)
			$0:=New object:C1471("sessions"; New object:C1471)
		End if 
	: ($1="process")
		$0:=Process activity:C1495(Processes only:K5:35)
	: ($1="userAndProcess")
		$0:=Process activity:C1495
		
	: ($1="maintenance")
		var $maintenance:=New object:C1471
		var $date1 : Date
		var $time1 : Time
		var $num1 : Integer
		var $text1 : Text
		BACKUP INFO:C888(Last backup date:K54:1; $date1; $time1)
		$maintenance.lastBackupText:=String:C10($date1)+" - "+String:C10($time1)
		BACKUP INFO:C888(Next backup date:K54:3; $date1; $time1)
		$maintenance.nextBackupText:=String:C10($date1)+" - "+String:C10($time1)
		BACKUP INFO:C888(Last backup status:K54:2; $num1; $text1)
		$maintenance.BackupStatus:=$text1
		
		
		var $compact:=Get 4D file:C1418(Compacting log file:K5:38)
		If ($compact#"")
			var $dom:=DOM Parse XML source:C719($compact)
			If (OK=0)
				$maintenance.compact:=Localized string:C991("MONI_MNTCE_VerificationLogDamaged")
			Else 
				var $Found:=DOM Find XML element:C864($dom; "verifylog/stop_timer")
				If (OK=1)
					var $time; $success; $cancel : Text
					...
					If ($cancel="true")
						$maintenance.compact:=$maintenance.compact+"  "+Localized string:C991("MONI_MNTCE_VerificationCanceledByUser")
					Else 
						If ($success="true")
							$maintenance.compact:=$maintenance.compact+"  "+Localized string:C991("MONI_MNTCE_CompactingSucceeded")
						Else 
							$maintenance.compact:=$maintenance.compact+"  "+Localized string:C991("MONI_MNTCE_CompactingFailed")
						End if 
					End if 
				End if 
			...
			End if 
		Else 
			$maintenance.compact:=Localized string:C991("MONI_MNTCE_Unknown")
		End if 
		
		
		var $verify:=Get 4D file:C1418(Verification log file:K5:37)
		If ($verify#"")
			$dom:=DOM Parse XML source:C719($verify)
			If (OK=0)
				$maintenance.verify:=Localized string:C991("MONI_MNTCE_VerificationLogDamaged")
			Else 
				...
				If (OK=1)
					...
					If ($cancel="true")
						$maintenance.verify:=$maintenance.verify+"  "+Localized string:C991("MONI_MNTCE_VerificationCanceledByUser")
					Else 
						If ($success="true")
							$maintenance.verify:=$maintenance.verify+"  "+Localized string:C991("MONI_MNTCE_VerificationSucceeded")
						Else 
							$maintenance.verify:=$maintenance.verify+"  "+Localized string:C991("MONI_MNTCE_VerificationFailed")
						End if 
					End if 
				End if 
				DOM CLOSE XML:C722($dom)
			End if 
		Else 
			$maintenance.verify:=Localized string:C991("MONI_MNTCE_Unknown")
		End if 
	...
	: ($1="SQL")
		...
		var $sqlconnectionhandler:=0
		var $obActivity:=Process activity:C1495
		var $i : Integer
	...
	: ($1="VerifyLog")
		...
		If ($verify#"")
			var $text:=Document to text:C1236($verify)
	...
	: ($1="applicationServer")
		...
		If (Is compiled mode:C492)
			$object.interpreted:=Localized string:C991("MONI_S4D_Compiled")
		Else 
			$object.interpreted:=Localized string:C991("MONI_S4D_Interpreted")
		End if 
		ARRAY TEXT:C222($arr1; 0)
		ARRAY REAL:C219($arr2; 0)
		ARRAY REAL:C219($arr3; 0)
		MEMORY STATISTICS:C1118(1; $arr1; $arr2; $arr3)
		var $col:=New collection:C1472
	...
	: ($1="web")
		...
		$0:=$object
	: ($1="DBMeasures")
		$0:=Database measures:C1314(New object:C1471("path"; "DB.tables"))
	: ($1="LockedRecords")
		$col:=New collection:C1472
		For ($i; 1; Last table number:C254)
			If (Is table number valid:C999($i))
				$object:=Locked records info:C1316(Table:C252($i)->)
